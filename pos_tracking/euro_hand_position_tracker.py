...
class OneEuro:
    def __init__(self, freq, min_cutoff=1.5, beta=0.4, d_cutoff=1.0):
        self.freq = float(freq)
        self.min_cutoff = float(min_cutoff)  # more = smoother at rest
        self.beta = float(beta)              # more = less lag when moving
...
class hand_position_tracker:
    ...
    def __init__(
        ...
        # after FPS is known:
        self.wrist_oe = OneEuro(freq=self.FPS, min_cutoff=2.6, beta=0.35, d_cutoff=1.2)
        self.landmark_oe = {}  # per-landmark OneEuro filters (key = idx)
        ...
        self.hands = self.mp_hands.Hands(
            ...
            min_detection_confidence=0.7,  # up from 0.6
            min_tracking_confidence=0.7,   # up from 0.6
            model_complexity=1             # 1 is a good balance; try 2 if GPU is comfy
        )
        ...
        self.max_jump_m = 0.08   # 8 cm per frame is already huge at 60 FPS
    ...
    def _pick_hand(self, results, prev_uv=None):
        # returns (landmarks, idx)
        if not results.multi_hand_landmarks:
            return None, -1
        if prev_uv is None:
            return results.multi_hand_landmarks[0], 0
        # pick the hand whose wrist (lm 0) is closest to prev_uv
        best = None; best_d = 1e9; best_i = -1
        for i, lms in enumerate(results.multi_hand_landmarks):
            u = float(lms.landmark[0].x) * self.COLOR_W
            v = float(lms.landmark[0].y) * self.COLOR_H
            d = (u - prev_uv[0])**2 + (v - prev_uv[1])**2
            if d < best_d:
                best_d, best, best_i = d, lms, i
        return best, best_i

    def _deadband_vec(self, v, eps=0.0015):  # ~1.5 mm
        v = np.asarray(v, np.float32)
        m = np.linalg.norm(v)
        return np.zeros_like(v) if m < eps else v
    ...
    def _landmarks_cam3d(self, df, lms):
        ...
        for i, lm in enumerate(lms.landmark):
            ...
            # before: p_s = self._ema(prev, p, self.PT_EMA_ALPHA)
            # after:
            flt = self.landmark_oe.get(i)
            if flt is None:
                flt = OneEuro(freq=self.FPS, min_cutoff=1.8, beta=0.5, d_cutoff=1.0)
                self.landmark_oe[i] = flt
            p_s = flt(p)
            
            #• If tiny tremors remain at rest, raise min_cutoff to ~2.2.
            #• If it lags when you move fast, raise beta to ~0.7.
            pts[i, :] = p_s
            ok_count += 1
    ...
    def start(self):
        ...
        self.depth_filters["holefill"] = rs.hole_filling_filter()
                ...
                # right after creating filters in start()
        sf = self.depth_filters["spatial"]
        sf.set_option(rs.option.holes_fill, 2)         # a bit stronger fill
        sf.set_option(rs.option.filter_magnitude, 2)   # edge-preserving strength
        sf.set_option(rs.option.filter_smooth_alpha, 0.5)
        sf.set_option(rs.option.filter_smooth_delta, 20)

        tf = self.depth_filters["temporal"]
        tf.set_option(rs.option.filter_smooth_alpha, 0.35)  # more smoothing
        tf.set_option(rs.option.filter_smooth_delta, 40)
    ...
    def process(self, draw_landmarks=True):
        ...
        # --- Depth filters (you already tuned options in start()) ---
        df = depth_frame
        ...
        # --- AprilTag pose for drawing only (capture_pose() does averaging) ---
        ...
        # Confidence gate: if low confidence, we hold last values
        conf_ok = True
        try:
        ...
        if res.multi_hand_landmarks and conf_ok:
            # Prefer the hand nearest to previous wrist pixel (if we have one)
            prev_uv = None
            if self._last_wrist is not None:
                try:
                    # project last wrist to pixel for continuity (x,y,z in meters)
                    last_xyz = self._last_wrist.reshape(3).astype(float).tolist()
                    uv = rs.rs2_project_point_to_pixel(self.intr, last_xyz)  # [u,v]
                    prev_uv = (float(uv[0]), float(uv[1]))
                except Exception:
                    prev_uv = None

            lms, _ = getattr(self, "_pick_hand", lambda r, pv: (r.multi_hand_landmarks[0], 0))(res, prev_uv)
            ...
            if lms is not None:
                ...
                # === All 21 landmarks as 3D camera points (meters) with One-Euro per point ===
                P_cam = self._landmarks_cam3d(df, lms)
                if P_cam is not None:
                    self._compute_flexions_deg(P_cam)  # updates self._angles

                # === Wrist-specific depth, One-Euro, jump clamp, world transform ===
                lm = lms.landmark[int(self.SHOW_LM)]
                u = float(lm.x) * self.COLOR_W
                v = float(lm.y) * self.COLOR_H
                z = self._depth_at_m(df, u, v, k=self.K_NEIGH)

                # Depth sanity & Z continuity check (Step 8)
                z_ok = (z > 0 and self.DEPTH_RANGE[0] <= z <= self.DEPTH_RANGE[1])
                if self._last_wrist is not None and z_ok:
                    last_z = float(self._last_wrist[2, 0])
                    if abs(z - last_z) > 0.12:  # >12 cm jump in one frame is suspicious
                        z_ok = False

                if z_ok:
                    # Camera-frame 3D from pixel+depth
                    p_cam_raw = np.array(
                        rs.rs2_deproject_pixel_to_point(self.intr, [u, v], float(z)),
                        dtype=np.float32
                    ).reshape(3, 1)

                    # One-Euro on wrist (Step 1)
                    p_cam = self.wrist_oe(p_cam_raw).reshape(3, 1)

                    # Max-jump clamp (Step 2) AFTER filtering
                    if self._last_wrist is not None:
                        jump = float(np.linalg.norm(p_cam.reshape(3) - self._last_wrist.reshape(3)))
                        if jump > float(self.max_jump_m):
                            p_cam = self._last_wrist.copy()

                    # Update last wrist now that it's valid
                    self._last_wrist = p_cam.copy()

                    # World transform if we have a pose
                    if self._have_pose:
                        ...
                        p_world = (Rwc @ p_cam + twc).reshape(3)

                        # --- Deadband in WORLD space (Step 3): place right HERE ---
                        if self._last_world_xyz is not None:
                            d = self._deadband_vec(p_world - np.asarray(self._last_world_xyz))
                            p_world = (np.asarray(self._last_world_xyz) + d).astype(np.float32)

                        world_xyz = p_world.copy()

                        # Normalize if limits exist
                        if all(v is not None for v in self.lims.values()):
                            ...
                            n = self._normalize_unit(world_xyz, pmin, pmax)
                    ...
                    # Depth not OK → hold last values if available
                    if self._last_world_xyz is not None:
                        world_xyz = np.asarray(self._last_world_xyz, dtype=np.float32)
                        nvec = self._last_norm_xyz

        # If no hands or confidence failed, hold last values (sticky output)
        if (not res.multi_hand_landmarks) or (not conf_ok):
            if self._last_world_xyz is not None:
                world_xyz = np.asarray(self._last_world_xyz, dtype=np.float32)
                nvec = self._last_norm_xyz

        # Commit outputs for getters/HUD
        self._last_world_xyz = None if world_xyz is None else world_xyz
