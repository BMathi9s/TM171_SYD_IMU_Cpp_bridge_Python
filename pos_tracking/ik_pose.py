...
class IKPoseController(Controller):
    ...
    Action layout (6 floats in [-1, 1]):
      0–2  absolute position around home: x, y, z  [m]
      3–5  absolute orientation (RPY):     roll, pitch, yaw  [rad]
    """
    ...
    def __init__(self, *, entity, model, **kwargs) -> None:
        ...
        # Simple ranges (absolute mapping from [-1,1] → around home)
        self.LIN_SCALE = 0.30          # ±30 cm around home
        self.ANG_SCALE = np.pi         # ±π rad (full turn)
    ...
    def _apply_cmd(self, vec) -> None:
        ...
        # 1) Map normalised action → absolute pose (around home)
        dp  = vec[:3] * self.LIN_SCALE             # XYZ [m]
        rpy = vec[3:] * self.ANG_SCALE             # RPY [rad]
        new_pos = self.home_pos + dp               # torch (3,)

        # 2) Build orientation from absolute RPY (SciPy needs CPU floats)
        new_R = R.from_euler("xyz", rpy.cpu().numpy())
        x, y, z, w = new_R.as_quat()               # SciPy gives [x,y,z,w]
        new_quat = torch.tensor([w, x, y, z], dtype=torch.float32, device=gs.device)

        # 3) IK + position control (same solver settings as your OSC)
